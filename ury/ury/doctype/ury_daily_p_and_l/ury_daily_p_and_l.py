...
def inner_bom_process(buying_price_list, bom, max_depth=2, current_depth=1):
    unset_bom_items = []
    bom_buying_price = 0
    ...
    for bom_item in bom.items:
        ...
        if len(boms) > 0 and current_depth < max_depth:
            inner_bom = frappe.get_doc("BOM", boms[0].name)
            inner_bom_data = inner_bom_process(buying_price_list, inner_bom, max_depth=max_depth, current_depth=current_depth + 1)
...
class URYDailyPandL(Document):
	...
	def get_proft_loss_details(self):
		# Dynamically fetch the company's default currency instead of hardcoding "INR"
		company = frappe.db.get_value("POS Profile", self.branch, "company") or frappe.defaults.get_global_default("company")
		currency = frappe.get_cached_value("Company", company, "default_currency") if company else "VND"
