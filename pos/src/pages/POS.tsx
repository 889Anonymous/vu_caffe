...
export default function POS() {
    ...
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-center">
          <p className="text-xl font-semibold text-red-600 mb-2">{t('errors.load_failed')}</p>
          <p className="text-gray-600">{error}</p>
          <button
            onClick={() => window.location.reload()}
            className="mt-4 px-4 py-2 bg-primary-600 text-white rounded-md hover:bg-primary-700"
          >
            {t('common.change')} {/* Temporarily using change or we can use generic retry, let's use t('common.reload') but vi.json doesn't have it. Let's use generic reload */}
          </button>
        </div>
      </div>
    );
  ...
  if (error) {
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center">
          <p className="text-lg font-medium text-red-600">{t('menu.error_loading')}</p>
        ...
        <div className="p-4 bg-white border-b border-gray-200">
          <div className="max-w-screen-xl mx-auto space-y-3">
            <div className="flex items-center gap-2 overflow-x-auto overflow-y-hidden">
              ...
              <QuickFilterButton filter="all" icon={Star} label={t('menu.all_categories')} />
              <QuickFilterButton filter="special" icon={TrendingUp} label={t('menu.variants')} />
            </div>
          </div>
        </div>
