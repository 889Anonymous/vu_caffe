...
const TableView = () => {
  ...
  useEffect(() => {
    async function fetchRooms() {
      ...
      try {
        ...
        console.error(e);
        setError(t('table.failed_load_rooms'));
      } finally {
  ...
  const loadTables = useCallback(
    async (roomName: string, options?: { useCache?: boolean }) => {
      ...
      try {
        ...
        console.error(e);
        setError(t('table.failed_load_tables'));
        setTables([]);
      } finally {
        setLoadingTables(false);
      }
  ...
  const handlePrintTable = async (table: Table, event: MouseEvent<HTMLButtonElement>) => {
    ...
    if (!posProfile) {
      showToast.error(t('table.pos_not_loaded'));
      return;
    }
    ...
    try {
      ...
      if (!invoiceId) {
        showToast.error(t('table.no_active_order'));
        return;
      }

      await printOrder({ orderId: invoiceId, posProfile });
      showToast.success(t('table.printed_success'));
      await loadTables(table.restaurant_room, { useCache: false });
    } catch (error) {
      showToast.error(error instanceof Error ? error.message : t('table.print_failed'));
    } finally {
              ...
              <div className="flex flex-wrap gap-2">
                {loadingRooms && (
                  <div className="flex-1 min-w-[160px]">
                    <Spinner message={t('table.loading_rooms')} />
                  </div>
                )}
      ...
      <div className="flex-1 overflow-auto bg-gray-50 p-6">
        <div className="max-w-screen-xl mx-auto h-full">
          {error && !loadingTables ? (
            ...
            <Spinner message={t('table.loading_tables')} />
          ) : tablesToDisplay.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center gap-3 text-gray-500">
              <Square className="w-10 h-10" />
              <p>{t('table.no_tables')}</p>
            </div>
          ) : (
                  ...
                  >
                    <div>
                      ...
                      <div className="space-y-2 text-sm text-gray-700">
                        ...
                        {isOccupied && (
                          <div className="flex items-center justify-between">
                            <span className="font-medium">{t('table.started_at')}</span>
                    ...
                    {isOccupied ? (
                      ...
                      <p className="text-sm text-gray-500">{t('table.tap_new_order')}</p>
                    )}
                  </div>
                );
              })}
