...
const PaymentDialog: React.FC<PaymentDialogProps> = ({
  ...
  const payments = paymentModes
    .map((mode: string | { id: string; name: string }) => {
    ...
    .filter(Boolean);
  const paymentsTotal = payments.reduce((sum, p: { mode_of_payment: string; amount: number } | null) => sum + (p?.amount || 0), 0);

  const handleApplyDiscount = () => {
    const value = parseFloat(discountValue);
    if (isNaN(value) || value <= 0) {
      setError(t('payment.invalid_discount'));
      return;
    }
    if (value > 100) {
      setError(t('payment.discount_exceed'));
      return;
    }
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (typeof window !== 'undefined' && (window as any).showToast) {
        (window as any).showToast.success(t('payment.payment_success'));
      }
      onClose();
      clearSelectedOrder();
      await fetchOrders();
      ...
      <DialogContent variant="xlarge" className="bg-white w-full max-w-4xl max-h-[90vh] flex flex-col md:flex-row p-0" showCloseButton={false}>
        ...
        <div className="md:w-1/2 p-6 border-b md:border-b-0 md:border-r border-gray-200 overflow-y-auto">
            ...
            <div className="space-y-4 mb-6">
              ...
              <div className="flex gap-2">
                <Input
                  ...
                  placeholder={t('payment.enter_percent')}
                  size="sm"
                  className="flex-1"
                />
                <Button
          ...
          <div className="space-y-4 mb-6">
            ...
            <div className="grid grid-cols-1 gap-3">
              {paymentModes.map((mode: string | { id: string; name: string }) => {
                const id = typeof mode === 'string' ? mode : mode.id;
                return (
        ...
        <div className="md:w-1/2 p-6 overflow-y-auto">
          ...
          <Button
            ...
            className="w-full"
          >
            {isProcessing ? t('payment.processing') : t('payment.pay', { amount: formatCurrency(paymentsTotal > 0 ? paymentsTotal : finalTotal) })}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
