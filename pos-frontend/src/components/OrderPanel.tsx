...
const OrderPanel = () => {
    ...
    try {
      ...
      if (selectedOrderType === 'Aggregators') {
        if (!selectedAggregator?.customer) {
          showToast.error(t('order_panel.select_aggregator'));
          return;
        }
      } else if (!selectedCustomer?.name) {
        showToast.error(t('order_panel.select_customer'));
        return;
      }

      // Validate table selection for dine-in orders
      if (selectedOrderType === DINE_IN && !selectedTable) {
        showToast.error(t('order_panel.select_table_for', { orderType: DINE_IN }));
        return;
      }
      ...
      resetOrderState();
      showToast.success(isUpdatingOrder ? t('order_panel.order_updated') : t('order_panel.order_submitted'));
    } catch (error) {
      ...
      if (error && typeof error === 'object' && '_server_messages' in error && typeof (error as any)._server_messages === 'string') {
        try {
          ...
          showToast.error(messageObj.message || t('order_panel.api_error'));
        } catch {
          showToast.error(t('order_panel.api_error'));
        }
      } else if (error instanceof Error) {
        showToast.error(error.message);
      } else {
        showToast.error(t('order_panel.failed_process_order'));
      }
    } finally {
  ...
  const EmptyCartUI = () => (
    <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
      ...
      <h3 className="text-lg font-semibold text-gray-900 mb-2">
        {t('order_panel.empty_cart')}
      </h3>

      <p className="text-gray-500 text-sm mb-6 max-w-xs leading-relaxed">
        {t('order_panel.empty_cart_hint')}
      </p>

      <div className="flex items-center gap-2 text-blue-600 bg-blue-50 px-4 py-2 rounded-lg">
        <Plus className="w-4 h-4" />
        <span className="text-sm font-medium">{t('order_panel.click_to_add')}</span>
      </div>

      <div className="mt-4 text-xs text-gray-400">
        {t('order_panel.double_click_hint')}
      </div>
    </div>
  );
  ...
  const LoadingOrderUI = () => (
    <div className="h-96">
      <Spinner message={t('order_panel.loading_order')} />
    </div>
  );
            ...
            <div className="flex justify-between items-center mb-4">
              <div className="flex items-center gap-2">
                <Button
                  ...
                  disabled={isInteractionDisabled}
                  title={orderComment ? t('order_panel.edit_comment') : t('order_panel.add_comment')}
                >
            ...
            >
              {isSubmitting ? (
                <div className="flex items-center">
                  ...
                  {isUpdatingOrder ? t('order_panel.updating_order') : t('order_panel.processing_order')}
                </div>
              ) : isUpdatingOrder ? (
                t('order_panel.update_order')
              ) : (
                t('order_panel.add_new_order')
              )}
            </Button>
          </div>
        </>
