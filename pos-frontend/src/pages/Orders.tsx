...
export default function Orders() {
  ...
  const formatDateTime = (date: string, time: string) => {
    const formattedDate = new Date(date + ' ' + time).toLocaleString('vi-VN', {
      month: 'short',
      day: 'numeric',
      hour: 'numeric',
      minute: 'numeric',
  ...
  // Map status values to translation keys for display
  const getStatusLabel = (status: string) => {
    const statusKey: Record<string, string> = {
      'Draft': 'orders.draft',
      'Unbilled': 'orders.unbilled',
      'Recently Paid': 'orders.recently_paid',
      'Paid': 'orders.paid',
      'Consolidated': 'orders.consolidated',
  ...
  async function handleCancelOrder() {
    ...
    if (!cancelReason.trim()) {
      showToast.error(t('orders.cancel_reason_required'));
      return;
    }
    ...
    try {
      ...
      showToast.success(t('orders.cancel_success'));
      setCancelDialogOpen(false);
      setCancelReason('');
      clearSelectedOrder();
      fetchOrders();
    } catch (err) {
      showToast.error(err instanceof Error ? err.message : t('orders.cancel_failed'));
    } finally {
  ...
  async function handleEditOrder() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(t('orders.failed_load_details'));
      ...
      showToast.error(err instanceof Error ? err.message : t('orders.edit_failed'));
    } finally {
  ...
  async function handlePrintOrder() {
    ...
    try {
      ...
      showToast.success(t('orders.print_success'));
      ...
      if (selectedStatus === 'Unbilled') {
        showToast.info(t('orders.moved_to_draft'));
        setSelectedStatus('Draft');
        fetchOrders();
      }
    } catch (err: any) {
      showToast.error(t('orders.print_failed') + ': ' + (err?.message || err));
    } finally {
      setIsPrinting(false);
    }
  }
          ...
          {orderLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-w-screen-xl mx-auto">
              {orders.map((order) => (
                <Card
                  key={order.name}
                  className={`p-0 bg-white hover:shadow-md transition-shadow flex flex-col overflow-hidden cursor-pointer ${selectedOrder?.name === order.name ? 'ring-2 ring-blue-500 shadow-lg' : ''
                    }`}
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-xs text-gray-500">
                            {order.restaurant_table ? `${order.restaurant_table} • ` : ''}{order.order_type}
                          </p>
                        </div>
      ...
      <div className="w-96 bg-white border-l border-gray-200 flex flex-col h-[calc(100vh-4rem)] fixed right-0 z-10">
        {!selectedOrder ? (
          <div className="text-center h-full flex flex-col items-center justify-center text-gray-500 p-6">
            <p className="text-lg font-medium mb-2">{t('orders.select_order')}</p>
            <p className="text-sm">{t('orders.select_order_hint')}</p>
          </div>
        ) : selectedOrderLoading ? (
          <div className="flex items-center justify-center h-full">
            <Spinner />
          ...
          <>
                ...
                {(selectedOrder.status === 'Draft' || selectedOrder.status === 'Unbilled' || selectedOrder.status === 'Recently Paid') && (
                  <>
                    ...
                    >
                      ...
                      {editLoading && <span className="ml-2 text-xs">{t('common.loading')}</span>}
                    </button>
                    <button
                      type="button"
                      ...
                      aria-label={t('orders.cancel_order')}
              ...
              <DialogContent>
                ...
                <DialogFooter>
                  ...
                  <Button variant="danger" onClick={handleCancelOrder} disabled={cancelLoading}>
                    {cancelLoading ? t('orders.cancelling') : t('orders.confirm_cancel')}
                  </Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
            ...
            <div className="flex-1 overflow-y-auto p-6 pb-40">
              ...
              {/* Order Items */}
              <div className="mb-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('orders.order_items')}</h3>
                <div className="space-y-3">
              ...
              {selectedOrderTaxes.length > 0 && (
                <div className="mb-6">
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('orders.taxes_charges')}</h3>
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (String(selectedOrder.invoice_printed) === '0') {
                        showToast.error(t('orders.print_before_payment'));
                        return;
                      }
